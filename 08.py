raw = '''gbcefa eac acfbg ae dcabfg begcdaf ecgba fgaedc beaf gcbde | cbgfa gedcb fgecab fbagdc
...
connections = [r[0].split() for r in raw]
output = [r[1].split() for r in raw]
values=[2,3,4,7]
...
for row in output:
    for w in row:
        if len(w) in values:
            part_one+=1
print(part_one)
#letter in 7 not in 1 is top 
#3 has 3 left after subtracting 1,
#bottom is 3-top-4,
#middle is 3-top-bottom-1
#top left is 4-1-middle
#bottom left is len(6)-top-bottom-1-top left-middle
#bottom right is len(5)-top-bottom-middle-topleft-bottomleft
#top right is other letter

def findThree(l,one):
    for word in l:
        if len(word) == 5:
            if len(set(word)-set(one)) == 3:
                return word

def findBottomLeft(l,t,b,m,tl,one):
    for word in l:
        if len(word) == 6:
            if len(set(word)-set(t)-set(b)-set(m)-set(tl)-set(one)) == 1:
                return list(set(word)-set(t)-set(b)-set(m)-set(tl)-set(one))[0]
                
def findBottomRight(l,t,b,m,tl):
    for word in l:
        if len(word) == 5:
            if len(set(word)-set(t)-set(b)-set(m)-set(tl)) == 1:
                return list(set(word)-set(t)-set(b)-set(m)-set(tl))[0]
...
def deduceDigits(l):
    ...
    for word in l:
        if len(word) == 2:
            ...
            d["".join(sorted(word))] = 7
            seven = word
        elif len(word) == 4:
            d["".join(sorted(word))] = 4 
            four = word
        elif len(word) == 7:
            d["".join(sorted(word))] = 8 
    
    top =list(set(seven)-set(one))[0]
    three = findThree(l,one)
    d["".join(sorted(three))] = 3
    bottom = list(set(three)-set(top)-set(four))[0]
    middle = list(set(three)-set(top)-set(bottom)-set(one))[0]
    top_left = list(set(four)-set(one)-set(middle))[0]
    bottom_left = findBottomLeft(l,top,bottom,middle,top_left,one)
    bottom_right = findBottomRight(l,top,bottom,middle,top_left)
    top_right = list(set(one)-set(bottom_right))[0]
    zero = top + bottom + top_left + bottom_left + top_right + bottom_right
    two = top + bottom + middle + top_right + bottom_left
    five = top + bottom + middle + top_left + bottom_right
    six = top + bottom + middle + top_left + bottom_left + bottom_right
    nine = top + bottom + middle + top_left + top_right + bottom_right
 
    for word in l:
        S = set(word)
        if S == set(zero):
            d["".join(sorted(word))] = 0
        elif S == set(two):
            d["".join(sorted(word))] = 2
        elif S == set(five):
            d["".join(sorted(word))] = 5
        elif S == set(six):
            d["".join(sorted(word))] = 6
        elif S == set(nine):
            d["".join(sorted(word))] = 9
    return d 
total = 0
for r in raw:
    d = deduceDigits(r[0].split())
    r[1] = r[1].split()
    total+= 1000*d["".join(sorted(r[1][0]))] + 100*d["".join(sorted(r[1][1]))] + 10*d["".join(sorted(r[1][2]))] +d["".join(sorted(r[1][3]))]
print(total)